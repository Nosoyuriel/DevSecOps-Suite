...
1.  **Modificar el script `initial-setup.sh`:**
    *   **Nombre y clave del proyecto:** Edita las siguientes líneas para establecer el nombre y la clave de tu proyecto.
        ```bash
...
*   `initial-setup.sh`: Inicia las herramientas desde cero, formatea SonarQube eliminando todos los proyectos y escaneos previos, y finalmente ejecuta los 3 tipos de análisis.
*   `scan.sh`: Realiza los escaneos de forma individual (`sast`, `dast`, `sca`).
*   `zero-all.sh`: Formatea las herramientas para eliminar los escaneos anteriores y ejecuta los 3 tipos de escaneo de forma escalonada.
*   `add-project.sh`: Agrega un nuevo proyecto a SonarQube para realizar un análisis estático.
*   `analyze-all.sh`: Ejecuta los 3 tipos de escaneo de forma escalonada sin eliminar los análisis anteriores.
